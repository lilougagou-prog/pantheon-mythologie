...
## Boîte de réception partagée avec l'app sœur Tarot de Delphes

Retour d'utilisatrice, après avoir demandé le même formulaire « Nous contacter » côté Tarot de Delphes : plutôt que deux boîtes de réception séparées, tous les messages des deux apps tombent désormais dans la **même** table `contact_messages`, distingués par une nouvelle colonne `app` (`'pantheon'` ou `'tarot'`) — jamais une valeur envoyée par le client, toujours fixée côté serveur par l'appelant. Migration défensive (`ALTER TABLE ... ADD COLUMN IF NOT EXISTS app ... DEFAULT 'pantheon'`) pour la table déjà existante ; les messages Panthéon déjà en base héritent silencieusement de cette valeur par défaut, sans script de rattrapage nécessaire.

Côté Tarot de Delphes (dépôt séparé, `lilougagou-prog/Tarot-mythologie`) : un formulaire identique dans l'esprit (type de demande, message, email) mais entièrement dans son propre vocabulaire visuel (palette crème/or, composants `.tile`/`.secondary`/`.primary` déjà existants, sélecteur de type calqué sur le toggle Se connecter/Créer un compte déjà présent sur l'écran Compte) — jamais un copier-coller de l'interface de Panthéon. Contrainte découverte en explorant ce dépôt : Tarot est déjà à la limite des 12 fonctions serverless du plan Hobby Vercel (`api/account.js` regroupe déjà 6 anciens endpoints pour cette même raison, voir son propre historique) — un 13ᵉ fichier `api/contact.js` aurait fait échouer le déploiement. Ajouté à la place comme une 7ᵉ action dans ce fichier existant (`?action=contact`), avec sa propre connexion Postgres **délibérément séparée** (`CONTACT_DATABASE_URL`, nouvelle variable, pointant vers la base de Panthéon) de celle qui gère ses comptes utilisateurs (`DATABASE_URL`, restée intacte) — jamais les deux bases mélangées.

Consultation : toujours uniquement depuis `admin-messages.html` (Panthéon) — Tarot n'a pas sa propre page de lecture, seulement l'écriture. La page gagne un badge d'origine par message (liseré de couleur + étiquette « Panthéon »/« Tarot de Delphes ») et trois filtres (Toutes / Panthéon / Tarot de Delphes) pour ne pas se retrouver avec un flux mélangé illisible.
